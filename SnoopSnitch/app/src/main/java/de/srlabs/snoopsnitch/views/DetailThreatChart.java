...
public class DetailThreatChart extends View {
	...
	public DetailThreatChart(Context context, AttributeSet attrs) {
		...
		if (_threatType == R.id.SilentSMSCharts) {
			_color = ContextCompat.getColor(context, R.color.common_chartYellow);
			_color_uploaded = ContextCompat.getColor(context, R.color.common_chartYellow_uploaded);
			//_imgThreatType.setBackgroundResource(R.drawable.ic_content_sms_event);
		} else {
			_color = ContextCompat.getColor(context, R.color.common_chartRed);
			_color_uploaded = ContextCompat.getColor(context, R.color.common_chartRed_uploaded);
			//_imgThreatType.setBackgroundResource(R.drawable.ic_content_sms_event);
		}
	...
	protected void drawChartColumn(float startX, float endX, int elementWidth, boolean[] items_upload_state, Canvas canvas) {
		...
		if (items_upload_state.length == 0) {
			...
			myPaint.setColor(ContextCompat.getColor(context, R.color.common_chartGreen));
