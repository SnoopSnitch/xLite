...
public class ActiveTestService extends Service{
	...
	private static final String TAG = "SNSN:SVC"; // old: "msd-active-test-service"
	...
	class MyActiveTestServiceStub extends IActiveTestService.Stub {
		...
		@Override
		public void setUploadDisabled(boolean uploadDisabled) throws RemoteException {
		...
		void iterate(){
			...
			if(continiousMode){
                ...
                if(nextState == State.SMS_MO){
					...
					// Wait 500 ms before closing the file to get a complete capture of all messages generated by the test.
	...
	private boolean startTest(String ownNumber){
		...
		try {
            ...
            Log.i(TAG, "Reflection of getITelephony seem OK.");
		} catch (Exception e) {
	...
	private void stopTest(){
		stateInfo(mTAG + "stopTest() called");
		testRunning = false;
		try{
			unregisterReceiver(smsReceiver);
		} catch (Exception e) {
            // unregisterReceiver throws an Exception if it isn't registered, so let's just ignore it.
			Log.w(TAG, mTAG + "Exception: receiver was not registered! Ignoring.");
		}
	...
	private void startExtraFileRecording(){
        ...
        // ToDo: What is the point of making such a complicated filename? Simplify!
        ...
        // ToDo: Clarify WTF the point of these logs? Are we recording to a file or not?
		if(uploadDisabled){
			stateInfo("Would now open dumpfile: " + filename);
            // Make sure MsdService keeps recording (for the local analysis) even if we don't upload anything
            msdServiceHelper.startActiveTest();
		} else{
			stateInfo("Opening ExtraRecording dumpfile: " + filename);
	...
	private void endExtraFileRecording(boolean upload){
		if(uploadDisabled){
			if(upload){
				stateInfo("Would now close and upload ExtraRecording file: " + currentExtraRecordingFilename);
			} else{
				stateInfo("Would now discard ExtraRecording file: " + currentExtraRecordingFilename);
			}
